...
- [ ] **1. Veri Modelleme ve Repository:**
  - [ ] `DailyLog` modelinin tanımlanması (tarih, miktar, ruh hali, kriz, notlar).
  - [ ] Hive entegrasyonu (local DB kurulumu `daily_logs` box).
  - [ ] Firestore entegrasyonu (uzak DB senkronizasyonu).
  - [ ] Riverpod `dailyLogsProvider`'ın yazılması.
- [ ] **2. Günlük Check-in UI (Uygulama Açılışı):**
  - [ ] Açılışta (bugün kayıt yoksa) tetiklenecek akışın hazırlanması.
  ...
  - [ ] Kaydet animasyonu ve veritabanı yazma işlemi.
- [ ] **3. Geçmiş Sayfası (History Screen):**
  - [ ] Sayfa iskeleti ve beyaz temiz kart tasarımının oluşturulması.
  - [ ] `fl_chart` paketi ile Haftalık/Aylık Sütun Grafiği (Bar Chart) kodlanması.
  - [ ] Günlük Log listesinin tasarımdaki gibi kartlar şeklinde (ikon, miktar, kısa not, kriz sayısı) dökülmesi.
  - [ ] Liste elemanına bastığında detayları açılan (Expandable) "Daha Fazla" görünümü.
- [ ] **4. Test ve Entegrasyon:**
  - [ ] Girilen check-in verisinin aynı anda Dashboard ve Grafik'e yansıması kontrolü.
  - [ ] Renk ve ikonların temaya uyumu.
